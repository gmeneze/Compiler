...
__global__ void norm(float *in, float *out, float *mul, int width){
	...
	sharedSum[threadIdx.y * BLOCK_SIZE + threadIdx.x] = in[start + threadIdx.y * width + threadIdx.x] * mul[threadIdx.x];

        // Optimization 1: Loop Unrolling and Sequential Addressing Reduction
        __syncthreads();
	...
	if(tx % 2 == 0 && ty % 2 == 0)
		out[tx * width + ty] = 2.0 * in[tx * width + ty]/sharedSum[0];
	else if(tx % 2 == 1 && ty % 2 == 0)
		out[tx * width + ty] = in[tx * width + ty]/sharedSum[0];
	else if(tx % 2 == 1 && ty % 2 == 1)
		out[tx * width + ty] = (-1.0) * in[tx * width + ty]/sharedSum[0];
	else
